...
알고리즘:
  ...
  2. tf2 로 optical → map 변환  (★ /tf→/j100_0915/tf 리매핑 필수, launch 에서 적용)
     - 젯슨↔mini PC 시간 비동기일 수 있어 기본은 "최신 TF"(stamp=0)로 lookup
...
class TagCollectorNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('use_latest_tf', True)     # 시간 비동기 회피용 (stamp=0 lookup)
    ...
    def to_map(self, tvec, optical_frame, stamp):
        ...
        # 시간 비동기 회피: stamp=0 → tf2 가 "최신" TF 사용
        ps.header.stamp = TimeMsg() if self.use_latest_tf else stamp
        ...
        except Exception as e:   # LookupException/ExtrapolationException 등
